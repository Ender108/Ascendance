## 1.1.0 - 4/1/2026

This update is NOT save safe.

### Added
- Creation Club - Dead Man's Dread
- Creation Club - Divine Crusader
- Creation Club - Hendraheim
- Creation Club - Necromantic Grimoire
- Faster Decompression
- SkyUI 3D Item Offset Fix
- STB Widgets
- STB Active Effects
- Widescreen Scale Removed for 1-6-1130 and higher
- Vel'dun UI
- Bethesda Plugin Manager - Extended
- Blazing's Saints and Seducer Material Rebalance
- Exist's Caves - PBR Retexture
- Foxes of Skyrim
- Volumetric Mists
- Obsidian Weathers and Seasons
- Obsidian Weathers and Seasons MCM Patch
- Obsidian CS
- Obsidian Weathers - Darker Nights Redone
- Particle Wind - SKSE Plugin
- Knight of the North - Divine Crusader Reworked
- Freak's Floral Meadows
- Freak's Floral Veil
- Freak's Floral Solstheim
- Faultier's PBR Skyrim AIO 2k
- Faultier's PBR Landscape 2k
- Faultier's PBR Armors and Clothes
- Alternative Armors PBR
- Silver Armor Redone PBR
- Divine Crusader - PBR Redone
- Redguard Elite Armaments PBR
- SmoothCam
- SmoothCam Vanilla Enhanced 2
- Spaghetti's Cities - AIO
- Spaghetti's Towns - AIO
- Spaghetti's Solstheim - AIO
- Spaghetti's Palaces - AIO
- Spaghetti's Orc Strongholds - AIO
- Spaghetti's Faction Halls - AIO
- Spaghetti's Faction Halls - AIO - No College of Winterhold
- Spaghetti's Mods - NavCut Addon
- Tomato's PBR Farmhouses 2.0
- True Directional Movement Mounted Archery Fix
- Hokoron - Enemy Overhaul
- Hokoron - Simonrim Patch
- Gesture Animation Remix (OAR)
- NPC Animation Remix (OAR)
- Conditional tavern cheering (OAR)
- Arm Movement Animations (OAR)
- Stones of Solitude - Better Blended Rock Piles
- Imperial Castles of Skyrim
- Mysterium Xarxes - A SimonRim Addon for The Cause
- Mysterium Xarxes - Sorcerer
- Blasphemous Revenants - A SimonRim Addon for Necromantic Grimoire and Plague of the Dead
- Skyrim Hearthfire Roads
- Homes Under the Warhammer - Hendraheim
- CC Hendraheim - Tweaks and Enhancements
- Hendraheim - TnE - Homes Under the Warhammer Patch
- Windhelm Bridge Reforged
- Windhelm Fake Windows Fix
- Blended Windhelm Bridge Snowdrifts
- A-Pose Bug Fix - Universal Behavior Runtime
- Pandora Behaviour Engine Plus

### Updated
- Alt-Tab Stuck Key Fix NG 1.3.1
- CC Myrwatch - Tweaks and Enhancements 2.0.1
- HorsePower - Modernized Horse Riding (Total Riding Overhaul) 1.1.1
- No Grass in Objects 1.6.3 Beta 3
- SkyUI v6.7
- SPID 7.2.0
- Survival Mode Improved - SKSE 1.6.4 RC 2
- Unofficial Skyrim Special Edition Patch - USSEP 4.3.8a
- Why I Came to Skyrim 2.01

### Removed
- Better Dialogue Controls
- Better MessageBox Controls
- Complete Widescreen Fix 219 for Vanilla and SkyUI
- Wider MCM Menu for SkyUI
- ReCleaned Menu
- QuickLoot IE Vanilla Skin - Updated for QLIE 3.3.0 and above
- Improved Cursors
- Menu and Load Smoke Removed
- Revert AE Main Menu Logo Change
- DEAD - Draugr Enemy Asset Diversity
- Draugr Enemy Asset Diversity Bugfix Patch
- FYX - Campfire Reacts to the Wind - EmbersXD Edition
- EMPEROR - Giant Crab Overhaul
- IHSS - Improved Horse Step Sounds
- Merethic Grasslands
- NAT.ENB III
- NAT.CS
- Faultier's individual retextures
- Riton's PBR Farmhouses 2k
- Vanaheimr Landscapes - AIO - PBR
- Vanaheimr - Mines and Caves - PBR
- Vanaheimr - Mines and Caves - Blending Meshes
- Vanaheimr - Ore Veins - PBR
- Realistic PBR Footprints - Vanaheimr Patch
- Stony AF Markarth and Dwemer Ruins 2K
- Stony AF Markarth PBR
- Remastered Dwarven Texture Tweaks
- Remastered Dwarven Texture Tweaks - SMIM Patch
- Tomato's Windhelm PBR - 2K
- Tomato's PBR Solitude 2k
- Why I Came to Skyrim - Select Your Own Quest
- Xavbio Weapon and Armor Retextures
- Tomato's Windhelm PBR - 2K
- Project New Reign - Nemesis Unlimited Behavior Engine
- Nemesis Creature Behaivour Compatiblity
- Nemesis Creature Behaivour - WereWolf Addon
- XEMI Utility
- Window Shadows Ultimate
- Window Shadows Ultimate - Patches
- Wyrmstooth
- Siege at Icemoth

## 1.0.1

This update is save safe.

### Tweaks
- FOV changed to 90
- Difficulty set to Expert instead of Master
- Disabled Bethesda.net platform from main menu
- Changed weight of Woodcutter's Axe and Pickaxe from 10 to 5 (includes unique variants)

### Added
- Pretty separator colors

### Removed
- CS Lights (too many lights)

## 1.0.0
Release.
